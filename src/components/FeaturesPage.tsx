...
import { Sparkles, Sliders, Cpu, Grid, Shield, Eye, RefreshCw, Layers, CheckCircle } from 'lucide-react';
    ...
    <div className="w-full min-h-screen bg-[#020617] text-[#F8FAFC] py-20 px-4 md:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-xs text-blue-400 font-mono font-bold mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-24 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            <div className="flex gap-1.5 p-1 bg-slate-950/80 border border-white/5 rounded-xl self-start mb-2">
              {categories.map(cat => (
                <button
                  key={cat.id}
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-5 rounded-2xl border transition-all cursor-pointer text-left relative overflow-hidden group ${
                        isSelected 
                          ? 'bg-[#111827]/60 border-blue-500/40 shadow-lg shadow-blue-500/5' 
                          : 'bg-[#111827]/25 border-white/5 hover:border-white/10 hover:bg-[#111827]/40'
                      }`}
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                          <h3 className="text-xs font-bold text-white tracking-wide font-sans">{f.title}</h3>
                          <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed">{f.description}</p>
                        </div>
                      </div>

                      {/* Accent gold dot or indicator on selected card */}
                      {isSelected && (
                        <div className="absolute top-3 right-3 w-1.5 h-1.5 rounded-full bg-[#FBBF24] shadow-[0_0_6px_#FBBF24]" />
                      )}
                    </motion.div>
                  );
                })}
          ...
          <div className="lg:col-span-7 bg-slate-950/60 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden text-left h-full">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div>
                    <span className="text-[9px] font-mono uppercase tracking-widest text-slate-400 bg-white/5 px-2 py-0.5 rounded">
                      MODULE: {activeFeatureObj.tag}
                    </span>
                    <h2 className="text-lg font-bold text-white mt-1.5 font-sans">{activeFeatureObj.title}</h2>
                  </div>

                  <div className="text-right">
                    <span className="text-2xl font-mono font-extrabold text-[#14B8A6]">{activeFeatureObj.metric}</span>
                ...
                <div className="space-y-3 pt-2">
                  <h4 className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">Engineering Specs:</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                    {activeFeatureObj.techSpecs.map((spec, index) => (
                      <div key={index} className="flex items-center gap-2 p-2.5 rounded-lg bg-white/5 border border-white/5 text-[11px] text-slate-200">
                ...
                <div className="pt-4 border-t border-white/5">
                  <div className="flex items-center justify-between text-[9px] font-mono text-slate-500 uppercase tracking-widest mb-3">
                    ...
                    <span className="text-[#14B8A6]">Processing Layer: 64ch</span>
                  </div>
                    ...
                    {['Raw Input', 'Feature Extraction', 'RRDB Blocks (x23)', 'Sub-Pixel Conv', 'Final Reconstruction'].map((step, idx) => (
                      <React.Fragment key={step}>
                        <div className={`p-1.5 rounded border text-[8px] font-mono text-center flex-1 transition-colors ${
                          idx === 2 
                            ? 'bg-gradient-to-r from-blue-500/10 to-teal-500/10 border-blue-500/30 text-white'
                            : 'bg-white/5 border-white/5 text-slate-400'
                        }`}>
                          {step}
                        </div>
                        {idx < 4 && <span className="text-slate-600 text-[9px]">→</span>}
                      </React.Fragment>
                    ))}
                  </div>
                </div>
        ...
        <div className="bg-[#111827]/20 border border-white/10 rounded-3xl p-6 md:p-8 shadow-xl text-left mb-16 relative">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 border-b border-white/10 pb-6">
            <div className="max-w-xl">
              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#FBBF24]">Interactive Comparison Simulator</span>
              <h2 className="text-xl md:text-2xl font-sans font-extrabold text-white mt-1 tracking-tight">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8 flex justify-center">
              <div className="relative w-full aspect-video rounded-2xl overflow-hidden border border-white/10 select-none max-w-3xl bg-slate-950">
                ...
                <div className="absolute inset-0 w-full h-full">
                  ...
                  />
                  <div className="absolute bottom-4 left-4 z-20 bg-slate-950/80 backdrop-blur-md border border-white/5 px-2.5 py-1 rounded-md text-[9px] font-mono text-slate-400 uppercase font-bold tracking-wider">
                    Compressed Low-Res Input
                  </div>
                </div>

                {/* Right Side: Enhanced Clear (Clipped dynamically based on slider position) */}
                <div 
                ...
                >
                  <img
                    ...
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute bottom-4 right-4 z-20 bg-gradient-to-r from-blue-500 to-teal-500 border border-blue-400/25 px-2.5 py-1 rounded-md text-[9px] font-mono text-white uppercase font-extrabold tracking-wider shadow-lg">
                ...
                <div 
                  className="absolute inset-y-0 w-0.5 bg-gradient-to-b from-blue-500 via-teal-400 to-blue-500 z-30 cursor-ew-resize"
                  style={{ left: `${sliderPosition}%` }}
                >
                  {/* Slider drag handle badge */}
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-950 border-2 border-teal-400 flex items-center justify-center text-teal-400 shadow-xl pointer-events-none">
            ...
            <div className="lg:col-span-4 space-y-5 text-left">
              <div className="bg-white/5 border border-white/5 p-4 rounded-2xl">
                ...
                <h3 className="text-sm font-bold text-white mt-2 font-sans">{comparisons[comparisonCategory].title}</h3>
                <p className="text-[11px] text-slate-400 mt-1 leading-relaxed font-sans">{comparisons[comparisonCategory].desc}</p>
              </div>
