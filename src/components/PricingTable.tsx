...
export default function PricingTable() {
  ...
  return (
    <div id="pricing" className="w-full py-20 px-4 md:px-8 border-b border-white/5 bg-slate-950 relative">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <p className="text-sm md:text-base text-slate-400 max-w-xl mb-8">
          ...
          {/* Toggle Switch */}
          <div className="flex items-center gap-3 bg-slate-900 border border-white/5 rounded-full p-1.5">
            <button
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-all cursor-pointer flex items-center gap-1.5 ${
                isAnnual ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white shadow-lg shadow-blue-500/15' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Yearly Billing
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch max-w-5xl mx-auto">
          {PRICING_PLANS.map((plan) => {
            ...
            return (
              <div
                key={plan.name}
                className={`bg-slate-900/40 backdrop-blur-md border rounded-2xl p-8 flex flex-col justify-between relative transition-all duration-300 hover:scale-[1.02] ${
                  plan.isPopular
                    ? 'border-cyan-500/30 shadow-[0_0_30px_rgba(20,184,166,0.08)]'
                    : 'border-white/5 hover:border-white/10'
                }`}
              >
                {/* Popular Pill Accent */}
                {plan.isPopular && (
                  <div className="absolute top-0 right-1/2 translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-blue-500 to-turquoise-400 text-slate-950 text-[10px] uppercase font-bold tracking-widest px-4 py-1 rounded-full shadow-lg">
                    Most Popular
                  </div>
                )}
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-slate-400 leading-normal mb-6 min-h-[36px]">{plan.description}</p>
                  ...
                  <ul className="flex flex-col gap-3.5 mb-8">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-2.5 text-xs text-slate-300">
                        <Check className="w-4 h-4 text-turquoise-400 shrink-0 mt-0.5" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-bold text-xs tracking-wider uppercase transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
                    plan.isPopular
                      ? 'bg-gradient-to-r from-blue-600 via-cyan-600 to-teal-600 hover:from-blue-500 hover:to-teal-500 text-white shadow-lg shadow-cyan-500/10'
                      : 'bg-slate-950 border border-white/10 text-slate-300 hover:text-white hover:border-white/20 hover:bg-slate-900'
                  }`}
                >
                  {plan.ctaText}
            ...
            <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden relative">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-slate-950/40">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-slate-200 p-1 rounded-lg border border-white/5 hover:border-white/10 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {paymentSuccess ? (
                <div className="p-8 text-center flex flex-col items-center">
                  ...
                  <button
                    onClick={handleCloseCheckout}
                    className="w-full py-2.5 bg-slate-950 border border-white/10 text-slate-200 hover:text-white text-xs font-semibold rounded-xl cursor-pointer"
                  >
                    Back to Application
                  </button>
                </div>
                ...
                <form onSubmit={handleProcessPayment} className="p-6 text-left">
                  ...
                  {/* Order Summary box */}
                  <div className="bg-slate-950/80 border border-white/5 rounded-xl p-4 mb-5">
                    ...
                    <div className="border-t border-white/5 my-3 pt-3 flex justify-between items-baseline">
                  ...
                  <div className="bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 text-[10px] p-3 rounded-lg mb-4 leading-normal flex items-start gap-1.5">
                    <ShieldCheck className="w-4.5 h-4.5 shrink-0 text-cyan-400 mt-0.5" />
                  ...
                  <div className="mb-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-slate-950 border border-white/5 rounded-xl px-3 py-2.5 text-xs text-slate-300 outline-none focus:border-cyan-500/50 transition-colors font-mono"
                      />
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      className="w-full py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold text-xs tracking-wider uppercase rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                    >
